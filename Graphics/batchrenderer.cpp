...
void BatchRenderer::submit(const Renderable& renderable)
{
	...
	m_Buffer->vertex = m_TransformationStack.back() * glm::vec4(position.x, position.y + size.y, 0.0f, 1.0f);
	m_Buffer->uv = uv[1];
	m_Buffer->tid = ts;
	m_Buffer->colour = color;
	++m_Buffer;
	...
	m_Buffer->vertex = m_TransformationStack.back() * glm::vec4(position.x + size.x, position.y + size.y, 0.0f, 1.0f);
	m_Buffer->uv = uv[2];
	m_Buffer->tid = ts;
	m_Buffer->colour = color;
	++m_Buffer;
	...
	m_Buffer->vertex = m_TransformationStack.back() * glm::vec4(position.x + size.x, position.y, 0.0f, 1.0f);
	m_Buffer->uv = uv[3];
	m_Buffer->tid = ts;
	m_Buffer->colour = color;
	++m_Buffer;
...
void BatchRenderer::drawString(const Font& font, const std::string& text, const glm::vec3& position, const glm::vec4& colour)
{
	...
	for (int i = 0; i < text.length(); i++)
	{
		...
		if (glyph != NULL)
		{
			...
			m_Buffer->vertex = glm::vec4(x0, y0, 0, 1);
			m_Buffer->uv = glm::vec2(u0, v0);
			m_Buffer->tid = ts;
			m_Buffer->colour = color;
			++m_Buffer;

			m_Buffer->vertex = glm::vec4(x0, y1, 0, 1);
			m_Buffer->uv = glm::vec2(u0, v1);
			m_Buffer->tid = ts;
			m_Buffer->colour = color;
			++m_Buffer;

			m_Buffer->vertex = glm::vec4(x1, y1, 0, 1);
			m_Buffer->uv = glm::vec2(u1, v1);
			m_Buffer->tid = ts;
			m_Buffer->colour = color;
			++m_Buffer;

			m_Buffer->vertex = glm::vec4(x1, y0, 0, 1);
...
void BatchRenderer::render(const Renderable& renderable)
{
	begin();
...
void BatchRenderer::render(const std::vector<Renderable>& renderables)
{
	begin();
