...
public:
	...
	void intersection(const std::vector<std::shared_ptr<Sprite>>& renderables, const BoundingBox& bbox)
	{
		glm::vec2 temp;
		float t1 = INT_MAX;
		float t2 = INT_MAX;
...
public:
	...
	void update(const std::vector<std::shared_ptr<Sprite>>& renderables, float timeElapsed);
	void render(Renderer& renderer);
