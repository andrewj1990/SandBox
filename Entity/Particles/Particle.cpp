...
Particle::Particle(float x, float y, float size, float angleDeg)
	: Entity(glm::vec3(x, y, 1000.0f), glm::vec2(size, size), glm::vec4(Utils::random(0.7f, 1.0f), Utils::random(0.5f, 1.0f), 0, rand() % 1000 / 1000.0f))
{
	...
	: Entity(glm::vec3(x, y, 1000.0f), glm::vec2(size, size), texture)
{
	...
	//m_Duration = Utils::random(0.2f, 1.0f);
	m_Duration = Utils::random(2.2f, 3.0f);
